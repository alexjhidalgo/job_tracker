...
function DeleteApplicationModal({ data, setData, showModal, closeModal, tableProps }) {
  const handleDelete = () => {
    fetch(`/applications/${tableProps.row.original.idCol}`, {
      method: "DELETE",
      headers: {
          ...
          >
            <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded">
              <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-slate-900">
                Are you sure you want to remove <b>{tableProps.row.original.positionCol}</b> at <b>{tableProps.row.original.companyCol}</b> from your applications?
              </Dialog.Title>
