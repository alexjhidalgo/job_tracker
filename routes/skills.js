...
router.get("/:application_skills",auth, function(req, res){
  const { application_skills } = body.params 
  const account_id = res.locals.user.id
  const text = 'SELECT DISTINCT skills."name","skill_id" FROM application_skills INNER JOIN skills ON application_skills.skill_id = skills.id  WHERE "application_id" = 2 and application_skills."account_id" = 1;'
  pool.query(text, [application_skills,account_id], (err, result) => {
    if (err) {
      res.status(400).send(err);
    }
    res.status(200).send(result.rows);
...
router.post("", auth, async (req, res) => {
  ...
  const getApplicationSkill = 'SELECT "id", "application_id","skill_id","account_id" FROM application_skills WHERE "application_id" = $1 and "skill_id" = $2 and "account_id" = $3;' 
